-- Lightweight storefront search RPC for Marketplace V1 discovery.
-- Returns matching public profiles only; no item joins.
-- Ranking: exact handle > handle prefix > other matches > alphabetical.

CREATE OR REPLACE FUNCTION public.search_storefronts(query text)
RETURNS TABLE (
  user_id       uuid,
  handle        text,
  display_name  text,
